...
@Path(ApiVersion.CURRENT + "(/{domain})?/path") public class WebResource {
    ...
    public Response getResource(@PathParam("domain") String domain, @PathParam("path") String path,
                                @HeaderParam("Accept") String accept, @Auth AuthorizationInfo authorizationInfo) {
        domain = getDomain(domain, authorizationInfo);
    ...
    private String getDomain(String uriDomain, AuthorizationInfo authorizationInfo){
        return (uriDomain!=null && !uriDomain.isEmpty()) ?
                uriDomain : authorizationInfo.getDomainId();
    }
}
