...
/**
 * Created by Francisco Sanchez on 22/04/15.
 */
public class I18nPutRemTest {
	...
	public void testPutResource() {
		...
		Optional<JsonElement> i18nCompleted = Optional.of(gson.toJsonTree(new I18n().setId(LANGUAGE + ":" + TEST_KEY)
				.setKey(TEST_KEY).setLang(LANGUAGE).setValue(TEST_VALUE)));

		when(remMock.resource(eq(TEST_COLLECTION_TYPE), eq(resourceId), eq(requestParametersMock), eq(i18nCompleted)))
				.thenReturn(responseMock);

		Response response = i18nPutRem.resource(TEST_COLLECTION_TYPE, resourceId, requestParametersMock,
				Optional.of(i18nTestModel));
