...
@RunWith(MockitoJUnitRunner.class) public class AclPostRemTest {
    ...
    public void setUp() throws Exception {
        ...
        when(aclResourcesService.saveResource(any(), any(), eq(TYPE), any(), any())).thenReturn(postResponse);
        when(aclResourcesService.updateResource(any(), eq(TYPE), any(), any(), any())).thenReturn(putResponse);
        rem = new AclPostRem(aclResourcesService, Collections.singletonList(putRem));
    ...
    public void testNoUserId() throws URISyntaxException {
        ...
        when(parameters.getTokenInfo()).thenReturn(tokenInfo);
        Response response = rem.collection(TYPE, parameters, new URI("test"), Optional.empty());
    ...
    public void testEmptyObject() throws URISyntaxException, IOException {
        ...
        Response response = rem.collection(TYPE, parameters, new URI("test"), Optional.of(entity));
    ...
    public void testCorrectNotJsonEntity() throws URISyntaxException {
        ...
        rem.collection(TYPE, parameters, new URI("test"), Optional.of(new ByteArrayInputStream(entity.toString().getBytes())));
        ...
        verify(aclResourcesService).saveResource(null, parameters, TYPE, uri, jsonObjectToSaveOnResmi);
    ...
    public void testCorrectJsonEntity() throws URISyntaxException {
        ...
        entity.add("_acl", acl);
        verify(aclResourcesService).saveResource(null, parameters, TYPE, uri, entity);
        verify(aclResourcesService, times(0)).updateResource(null, TYPE, new ResourceId(ID), null, entity);
    }
