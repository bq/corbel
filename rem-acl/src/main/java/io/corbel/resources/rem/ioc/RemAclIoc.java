...
@Configuration @Import({ConfigurationIoC.class, EventBusListeningIoc.class}) public class RemAclIoc {
    ...
    public EventHandler<ResourceEvent> getAclConfigurationEventHandlerAsEventHandler() {
        List<Pair<Rem, HttpMethod>> remsAndMethods = Arrays.asList(Pair.of(getAclPostRem(), HttpMethod.POST),
                Pair.of(getAclPutRem(), HttpMethod.PUT), Pair.of(getAclGetRem(), HttpMethod.GET),
                Pair.of(getAclDeleteRem(), HttpMethod.DELETE));

        return new AclConfigurationEventHandler(remsAndMethods, env.getProperty("rem.acl.configuration.collection"));
    }
