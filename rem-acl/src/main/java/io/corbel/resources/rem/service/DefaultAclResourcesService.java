...
public class DefaultAclResourcesService implements AclResourcesService {
    ...
    public boolean isAuthorized(TokenInfo tokenInfo, String type, ResourceId resourceId, AclPermission operation) {
        return isAuthorized(Optional.ofNullable(tokenInfo.getUserId()), tokenInfo.getGroups(), type, resourceId, operation);
    }
    ...
    @Override
    public Optional<JsonObject> getResourceIfIsAuthorized(TokenInfo tokenInfo, String type, ResourceId resourceId,
            AclPermission operation) {
        return getResourceIfIsAuthorized(Optional.ofNullable(tokenInfo.getUserId()), tokenInfo.getGroups(), type, resourceId, operation);
    }
