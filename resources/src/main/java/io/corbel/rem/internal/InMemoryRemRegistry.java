...
public class InMemoryRemRegistry implements RemRegistry {
    ...
    volatile private MemoryRegistry memoryRegistry = new MemoryRegistry(new TreeSet<>(), new HashMap<>());

    @Override
    public Rem getRem(String uri, List<MediaType> acceptableMediaTypes, HttpMethod method, List<Rem> remsExcluded) {
        MemoryRegistry currentMemoryRegistry = memoryRegistry;

        for (UriPatternRegistryEntry entry : currentMemoryRegistry.getRegistry()) {
    ...
    synchronized public void unregisterRem(Class<?> remClass, String uriPattern, MediaType mediaType) {
        ...
        Optional.ofNullable(registryLookUp.get(uriPattern)).ifPresent(registryEntry -> {
            SortedSet<UriPatternRegistryEntry> registry = new TreeSet<>(currentMemoryRegistry.getRegistry());

            if (registryEntry != null && registryEntry.removeAndCheckIfEmpty(remClass, mediaType)) {
                registryLookUp.remove(uriPattern);
                registry.remove(registryEntry);
            }

            memoryRegistry = new MemoryRegistry(registry, registryLookUp);
        });
    }

    @Override
    public List<RemDescription> getRegistryDescription() {
        SortedSet<UriPatternRegistryEntry> currentRegistry = memoryRegistry.getRegistry();
        List<RemDescription> descriptionsList = new LinkedList<>();

        for (UriPatternRegistryEntry uriPatternRegistryEntry : currentRegistry) {
