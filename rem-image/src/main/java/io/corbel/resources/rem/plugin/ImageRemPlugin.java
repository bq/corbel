package io.corbel.resources.rem.plugin;

import io.corbel.resources.rem.*;
import io.corbel.resources.rem.ioc.RemImageIoc;
import io.corbel.resources.rem.ioc.RemImageIocNames;
import io.corbel.lib.config.ConfigurationHelper;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.context.annotation.AnnotationConfigApplicationContext;
import org.springframework.http.HttpMethod;
import org.springframework.http.MediaType;
import org.springframework.stereotype.Component;

@Component
public class ImageRemPlugin extends RemPlugin {

    private static final Logger LOG = LoggerFactory.getLogger(ImageRemPlugin.class);
    private static final String IMAGE_CACHE_COLLECTION = "image.cache.collection";
    private static final String IMAGE_PATH = "image/*";
    public static final String START_URI_PATTERN = "^(?!";
    public static final String END_URI_PATTERN = "$).*";
    private final String ARTIFACT_ID = "rem-image";

    @Override
    protected void init() {
        LOG.info("Initializing Image plugin.");
        super.init();
        ConfigurationHelper.setConfigurationNamespace(ARTIFACT_ID);
        context = new AnnotationConfigApplicationContext(RemImageIoc.class);
    }

    @Override
    protected void register(RemRegistry registry) {
        registerRem(RemImageIocNames.REM_GET,HttpMethod.GET, registry);
        registerRem(RemImageIocNames.REM_PUT,HttpMethod.PUT, registry);
        registerRem(RemImageIocNames.REM_DELETE,HttpMethod.DELETE, registry);
    }

    private void registerRem(String remImageIocNames, HttpMethod httpMethod, RemRegistry registry) {
        ImageGetRem beanImageRem = (ImageGetRem) context.getBean(remImageIocNames, Rem.class);
        beanImageRem.setRemService(remService);
        registry.registerRem(beanImageRem, START_URI_PATTERN + context.getEnvironment().getProperty(IMAGE_CACHE_COLLECTION) + END_URI_PATTERN,
                MediaType.parseMediaType(IMAGE_PATH), httpMethod);
    }

    @Override
    protected String getArtifactName() {
        return ARTIFACT_ID;
    }

}
