...
public class DefaultImageCacheService implements ImageCacheService {
    ...
    public void removeFromCollectionCache(Rem<InputStream> restorDeleteRem, RequestParameters<CollectionParameters> parameters,
        ...
        restorDeleteRem.collection(cacheCollection,
                getCollectionParameters(getCachePrefix(parameters.getRequestedDomain(), collection), parameters), null, Optional.empty());
    }
    ...
    public void removeFromCache(Rem<InputStream> restorDeleteRem, RequestParameters<ResourceParameters> parameters, ResourceId resourceId,
            String collection) {
        restorDeleteRem.collection(cacheCollection,
                getCollectionParameters(getCachePrefix(parameters.getRequestedDomain(), resourceId.getId(), collection), parameters), null,
                Optional.empty());
    }
    ...
    private String getCachePrefix(String domain, String resourceId, String collection) {
        return domain + '/' + cacheCollection + '/' + getPrefix(resourceId, collection);
    }

    private String getCachePrefix(String domain, String collection) {
        return domain + '/' + cacheCollection + '/' + collection;
    }
}
