...
public class ElasticSearchResmiSearch implements ResmiSearch {
    ...
    public boolean upsertResmiIndex(String name, String settings) {
        if (!elasticSearchService.indexExists(name)) {
            String indexName = (name + clock.millis()).toLowerCase();
            elasticSearchService.createIndex(indexName, new Scanner(getClass().getResourceAsStream(settings), "UTF-8").useDelimiter("\\A")
                    .next());
            elasticSearchService.addAlias(indexName, name);
            return false;
        }
        return true;
    }
    ...
    public JsonArray search(ResourceUri uri, String templateName, Map<String, Object> templateParams, int page, int size) {
        ...
        if (upsertResmiIndex(elasticSearchType)) {
            return elasticSearchService.search(elasticSearchType, elasticSearchType, templateName, templateParams, page, size);
        } else {
    ...
    public AggregationResult count(ResourceUri uri, String templateName, Map<String, Object> templateParams) {
        String elasticSearchType = getElasticSearchType(uri);
        if (upsertResmiIndex(elasticSearchType)) {
            return elasticSearchService.count(elasticSearchType, elasticSearchType, templateName, templateParams);
        } else {
    ...
    public JsonArray search(ResourceUri uri, String search, Optional<List<ResourceQuery>> queries, Pagination pagination,
        ...
        String elasticSearchType = getElasticSearchType(uri);
        if (upsertResmiIndex(elasticSearchType)) {
            return elasticSearchService.search(elasticSearchType, elasticSearchType, search, queries.orElseGet(Collections::emptyList),
                    pagination, sort);
        } else {
            return new JsonArray();
        }
    ...
    public AggregationResult count(ResourceUri uri, String search, Optional<List<ResourceQuery>> queries) {
        String elasticSearchType = getElasticSearchType(uri);
        if (upsertResmiIndex(elasticSearchType)) {
            return new CountResult(elasticSearchService.count(elasticSearchType, elasticSearchType, search,
    ...
    public void indexDocument(ResourceUri uri, JsonObject fields) {
        Optional.ofNullable(uri.isResource() ? uri.getTypeId() : uri.getRelationId()).map(JsonPrimitive::new).ifPresent(resourceId -> {
            ...
            elasticSearchService.indexDocument(elasticSearchType, elasticSearchType, getElasticSearchId(uri), fields.toString());
        });
    }
    ...
    public void deleteDocument(ResourceUri uri) {
        String elasticSearchType = getElasticSearchType(uri);
        if (upsertResmiIndex(elasticSearchType)) {
            elasticSearchService.deleteDocument(elasticSearchType, elasticSearchType, getElasticSearchId(uri));
        }
    }
