...
public class DefaultResmiService implements ResmiService {
    ...
    public JsonObject updateResource(ResourceUri uri, JsonObject jsonObject) throws StartsWithUnderscoreException {
        ...
        JsonObject objectToVerifyDates = resmiDao.findResource(uri);

        if (objectToVerifyDates == null || objectToVerifyDates.has(_CREATED_AT)) {
            return jsonObject;
        }

        JsonObject newObjectToSetDates = new JsonObject();
        createDates(newObjectToSetDates);
        createDates(jsonObject);
        resmiDao.updateResource(uri, newObjectToSetDates);
