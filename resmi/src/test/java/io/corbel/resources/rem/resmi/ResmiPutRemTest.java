...
public class ResmiPutRemTest extends ResmiRemTest {
    ...
    public void testPutRelationWithUnderscoreInAttributeNameTest() throws NotFoundException, StartsWithUnderscoreException {
        JsonObject json = new JsonObject();
        json.add("_b", new JsonPrimitive("2"));
        Optional<JsonObject> optionalJson = Optional.ofNullable(json);

        doThrow(new StartsWithUnderscoreException("_b")).when(resmiServiceMock).createRelation(any(), eq(optionalJson));

        @SuppressWarnings("unchecked")
        Response response = putRem.relation(TEST_TYPE, TEST_ID, TEST_RELATION, getParameters(TEST_URI), optionalJson);
        assertThat(response.getStatus()).isEqualTo(422);
    }
