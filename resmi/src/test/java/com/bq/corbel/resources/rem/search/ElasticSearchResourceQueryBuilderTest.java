...
public class ElasticSearchResourceQueryBuilderTest {
    ...
    @Test
    public void withFieldsResourceQueryTest() throws MalformedJsonQueryException {
        List<String> fields = new ArrayList<String>();
        fields.add("name");
        fields.add("author");

        Search searchWithFields = new Search(false, SEARCH, fields);
        QueryBuilder query = ElasticSearchResourceQueryBuilder.build(searchWithFields, queryParser.parse("[{\"$eq\":{\"name\":\"Metallica\"}}]"));

        JsonNode result = jsonParser.readValueAsTree(query.toString());
        JsonNode jsonNode = result.get("bool").get("should");
        assertEquals("{\"query_string\":{\"query\":\"search string\",\"fields\":[\"name\",\"author\"]}}", jsonNode.get("bool").get("must").toString());
