...
@Path(ApiVersion.CURRENT + "/event") public class EventResource {
    ...
    public Response registerFormParamsEvent(@Auth AuthorizationInfo authorizationInfo, @PathParam("type") String type, Form params, @Context Request request) {
        Response response = Response.fromResponse(registerJsonEvent(authorizationInfo, type, getNodeParams(params))).type(MediaType.TEXT_PLAIN_TYPE)
                .entity("[accepted]").build();
        return response;
    }

    private JsonNode createEworkerMessage(AuthorizationInfo authorizationInfo, JsonNode event) {
        Header header = new Header(authorizationInfo.getDomainId(), authorizationInfo.getClientId(),
                authorizationInfo.getUserId(), authorizationInfo.getTokenReader().getInfo().getDeviceId());
