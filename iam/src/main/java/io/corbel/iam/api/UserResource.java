...
@Path(ApiVersion.CURRENT + "/user") public class UserResource {
    ...
    @GET
    @Path("/{id}/session")
    public Response getSession(@PathParam("id") String userId, @Auth AuthorizationInfo authorizationInfo){
        return resolveMeIdAliases(userId, authorizationInfo)
                .filter(user -> userDomainMatchAuthorizationDomain(user, authorizationInfo))
                .map(user -> {
                    return Response.ok()
