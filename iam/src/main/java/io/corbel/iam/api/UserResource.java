...
@Path(ApiVersion.CURRENT + "/{domain}/user") public class UserResource {
    ...
    public Response getDevices(@Rest QueryParameters queryParameters, @PathParam("domain") String domainId,
                               @PathParam("userId") String userId, @Auth AuthorizationInfo authorizationInfo) {
        User user = getUserResolvingMeAndUserDomainVerifying(userId, authorizationInfo.getUserId(), domainId);
        return Optional.ofNullable(deviceService.getByUserId(user.getId(), queryParameters)).map(devices -> {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response getDevice(@PathParam("domain") String domainId, @PathParam("userId") String userId, @PathParam("deviceId") String deviceUid,
            @Auth AuthorizationInfo authorizationInfo) {
        User user = getUserResolvingMeAndUserDomainVerifying(userId, authorizationInfo.getUserId(), domainId);
        return Optional.ofNullable(deviceService.getByUidAndUserId(deviceUid, user.getId(), user.getDomain())).map(device -> {
    ...
    @Path("/{userId}/devices")
    @Consumes(MediaType.APPLICATION_JSON)
    public Response updateDevice(@PathParam("domain") String domainId, @PathParam("userId") String userId, @Valid Device deviceData,
                                 @Auth AuthorizationInfo authorizationInfo, @Context UriInfo uriInfo) {
        User user = getUserResolvingMeAndUserDomainVerifying(userId, authorizationInfo.getUserId(), domainId);
    ...
    @PUT
    @Path("/{id}/disconnect")
    public Response disconnect(@PathParam("domain") String domainId, @PathParam("id") String userId, @Auth AuthorizationInfo authorizationInfo) {
        User user = getUserResolvingMeAndUserDomainVerifying(userId, authorizationInfo.getUserId(), domainId);
    ...
    @DELETE
    @Path("/{id}/sessions")
    public Response deleteAllSessions(@PathParam("domain") String domainId, @PathParam("id") String userId, @Auth AuthorizationInfo authorizationInfo) {
        User user = getUserResolvingMeAndUserDomainVerifying(userId, authorizationInfo.getUserId(), domainId);
    ...
    @GET
    @Path("/{id}/identity")
    public Response getUserIdentity(@PathParam("domain") String domainId, @PathParam("id") String id, @Auth AuthorizationInfo authorizationInfo) {
        User user = getUserResolvingMeAndUserDomainVerifying(id, authorizationInfo.getUserId(), domainId);
