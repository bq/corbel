...
public class DefaultAuthorizationService implements AuthorizationService {
    ...
    public TokenGrant authorize(String assertion) throws UnauthorizedException, MissingOAuthParamsException,
        ...
        try {
            ...
            throwMatchingException(assertion, e);
        }
        return null;
    }
    ...
    private void throwMatchingException(String assertion, RuntimeException e) throws BadSystemClockException, UnauthorizedException {
        logInvalidAssertion(assertion, e);
        if (isABadClockSystemException(e)) {
            throw new BadSystemClockException();
        } else {
            throw new UnauthorizedException("Invalid assertion");
        }
    }

    private boolean isABadClockSystemException(RuntimeException e) {
        return Optional.ofNullable(e.getMessage()).map(error -> error.contains("Invalid iat and/or exp.")).orElse(false);
    }
