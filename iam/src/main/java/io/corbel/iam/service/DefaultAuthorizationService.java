...
public class DefaultAuthorizationService implements AuthorizationService {
    ...
    /**
     * Original function: https://github.com/slategroup/jsontoken-osgi/blob/master/src/main/java/net/oauth/jsontoken/JsonTokenParser.java
     * Function appears duplicated here due to original thrown exception's ambiguity.
     * This way the error can be controlled in a isolated environment to changes and/or undesired exceptions.
     */
    private boolean isAnExpiredException(String assertion) {
        JsonToken unverifiedToken = jsonTokenParser.deserialize(assertion);
        org.joda.time.Instant issuedAt = unverifiedToken.getIssuedAt();
        org.joda.time.Instant expiration = unverifiedToken.getExpiration();
        SystemClock clock = new SystemClock();
        return (issuedAt.isAfter(expiration) || !clock.isCurrentTimeInInterval(issuedAt, expiration));
    }
