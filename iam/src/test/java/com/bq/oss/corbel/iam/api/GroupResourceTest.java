...
public class GroupResourceTest {
    ...
    public void deleteUnauthorizedGroupTest() {
        String anotherDomain = "anotherDomain";
        Group group = new Group(ID, NAME, anotherDomain, SCOPES);
    ...
    @Test
    public void updateGroupScopesTest() throws JsonProcessingException, NoGroupException {
        Set<String> scopes = new HashSet<>(Arrays.asList("scope1", "scope2"));
        String scopesJson = new ObjectMapper().writer().writeValueAsString(scopes);

        assertThat(RULE.client().target("/" + ApiVersion.CURRENT + "/group/" + ID).request().put(Entity.json(scopesJson), Response.class)
                .getStatus()).isEqualTo(Response.Status.NO_CONTENT.getStatusCode());

        verify(groupService).updateScopes(eq(ID), eq(scopes));
        verifyNoMoreInteractions(groupService);
    }

    @Test
    public void updateNonexistentGroupScopesTest() throws GroupAlreadyExistsException, NoGroupException, JsonProcessingException {
        Set<String> scopes = new HashSet<>(Arrays.asList("scope1", "scope2"));
        String scopesJson = new ObjectMapper().writer().writeValueAsString(scopes);

        doThrow(new NoGroupException(ID)).when(groupService).updateScopes(eq(ID), eq(scopes));
