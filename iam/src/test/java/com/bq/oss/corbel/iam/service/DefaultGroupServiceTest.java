...
@RunWith(MockitoJUnitRunner.class) public class DefaultGroupServiceTest {
    ...
    public void getAllGroupsTest() {
        ...
        group2.setDomain("anotherDomain");

        List<ResourceQuery> resourceQueries = Collections.emptyList();

        when(groupRepository.find(resourceQueries, null, null)).thenReturn(Arrays.asList(group1, group2));
    ...
    @Test
    public void updateGroupTest() throws NoGroupException, GroupAlreadyExistsException {
        Group group = getGroup();
        group.setName(null);
    ...
    @Test
    public void updateGroupWithNameTest() throws NoGroupException, GroupAlreadyExistsException {
        Group group = getGroup();

        when(groupRepository.findByIdAndDomain(ID, DOMAIN)).thenReturn(group);
        ...
        verify(groupRepository).findByIdAndDomain(ID, DOMAIN);
        verify(groupRepository).save(Mockito.<Group>any());
        verify(groupRepository).deleteByIdAndDomain(ID, DOMAIN);
        verify(groupRepository).patch(any());
        verifyNoMoreInteractions(groupRepository);
    }
    ...
    public void updateAlreadyExistingGroupWithNameTest() throws NoGroupException, GroupAlreadyExistsException {
        Group group = getGroup();

        when(groupRepository.findByIdAndDomain(ID, DOMAIN)).thenReturn(null);
    ...
    @Test(expected = GroupAlreadyExistsException.class)
    public void updateAlreadyExistingGroupTest() throws NoGroupException, GroupAlreadyExistsException {
        Group group = getGroup();

        when(groupRepository.findByIdAndDomain(ID, DOMAIN)).thenReturn(group);
        when(groupRepository.save(Mockito.<Group>any())).thenThrow(new DataIntegrityViolationException(""));
    ...
    @Test(expected = NoGroupException.class)
    public void updateNonExistentGroupTest() throws NoGroupException, GroupAlreadyExistsException {
        Group group = getGroup();
        group.setName(null);
