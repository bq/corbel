...
@RunWith(MockitoJUnitRunner.class) public class DefaultGroupServiceTest {
    ...
    @Test
    public void updateGroupScopesTest() throws NoGroupException {
        Set<String> scopes = new HashSet<>(Arrays.asList("scope1", "scope2"));
        Group expectedGroup = new Group(ID, null, null, scopes);

        when(groupRepository.patch(eq(expectedGroup))).thenReturn(true);

        groupService.updateScopes(ID, scopes);

        verify(groupRepository).patch(eq(expectedGroup));
        verifyNoMoreInteractions(groupRepository);
    }
    ...
    public void updateNonExistentGroupTest() throws NoGroupException {
        ...
        when(groupRepository.patch(eq(expectedGroup))).thenReturn(false);
